            ...
            <Box display={'flex'} flexDirection={'column'} alignItems={'center'} width={'100%'} overflow={'hidden'}>
                <Box sx={{ objectFit: 'cover', justifyContent: 'center' }}>
                    {product.photos && product.photos[mainPhotoId] && (
                        <img style={{maxWidth:'100%'}} src={`http://localhost:5555/${product.photos[mainPhotoId].file_path}`} />
                    )}
                </Box>

                <Box display={'flex'} flexDirection={'row'} justifyContent={'space-around'} marginTop={'1rem'}>
                    {product.photos?.slice(0, 2).map((photo, index) => (<Box key={photo.id} sx={{ borderStyle: 'solid' }}> <img style={{ height: 'auto', width: '60px' }} src={`http://localhost:5555/${photo.file_path}`} onClick={() => handleClick(index)} /> </Box>))}
                </Box>
                <Box >
                    ${product.price}
                </Box>
